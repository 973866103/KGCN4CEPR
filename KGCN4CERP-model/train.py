...
def train(args, data, show_loss, show_topk):
    ...
    with tf.Session() as sess:
        ...
        for step in range(args.n_epochs):
            ...
            test_auc, test_auc, test_f1 = ctr_eval(sess, model, test_data, args.batch_size)

            print('epoch %d    train auc: %.4f  acc: %.4f  f1: %.4f    eval auc: %.4f  acc: %.4f  f1: %.4f    test '
                  'auc: %.4f  acc: %.4f  f1: %.4f '
                  % (step, train_auc, train_acc, train_f1, eval_auc, eval_acc, eval_f1, test_auc, test_auc, test_f1))
